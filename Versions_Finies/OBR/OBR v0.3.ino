...
void Verification_Commande()
{
  ...
  if(stringComplete)
  {
    // S'il y a une commande pour nous, on l'éxécute
    if(NoDestinataire == MAITRE_ESCLAVE)
      ExecuterCommande();

    // Si une commande doit être transmise, on la transmet
    else
    {
      // On reconstitue la chaine
      ReconstituerCommande();
  ...
  if(stringComplete)
  {
    // On regarde si la commande est pour nous
    if(NoDestinataire==MAITRE_ESCLAVE)
      ExecuterCommande();

    // Sinon, on transmet le message à son destinataire (maitre, PC ou télécommande)
    else
    {
      // On reconstitue la chaine
      ReconstituerCommande();
...
// Fonction de reconstitution d'une commande à partir des variables
void ReconstituerCommande()
{
  // On reconstitue la chaine
  Commande = '['+String(NoEnvoyeur)+'|'+String(NoDestinataire)+']'+String(Commande);
  if(NbParam>0)
    Commande += String(param1);
  if(NbParam>1)
    Commande += String(param2);
  if(NbParam>2)
    Commande += String(param3);
}
...
void Envoie_Connexion(byte NoPort)
{
  ...
  else if(Commande==String("OBR_CONNECTER_PC"))
  {
    Ordinateur = NoPort;
    EnvoieMessage(PC, "OK");
  }

  else if(Commande==String("OBR_CONNECTER_TELECOMMANDE"))
  {
    Telecommande = NoPort;
    EnvoieMessage(TELECOMMANDE, "OK");
  }

  else if(Commande==String("OBR_CONNECTER_EXTENSION1"))
  {
    Extension1 = NoPort;
    EnvoieMessage(EXTENSION1, "OK");
  }

  else if(Commande==String("OBR_CONNECTER_EXTENSION2"))
  {
    Extension2 = NoPort;
...
void Demande_Connexion(byte NoPort)
{
  ...
  if(stringComplete)
  {
    ...
    if(Commande==String("OBR_CONNECTER_TELECOMMANDE") && !Telecommande)
    {
      Telecommande = NoPort;
      EnvoieMessage(TELECOMMANDE, "OK");
    }

    if(Commande==String("OBR_CONNECTER_EXTENSION1") && !Extension1)
    {
      Extension1 = NoPort;
      EnvoieMessage(EXTENSION1, "OK");
    }

    if(Commande==String("OBR_CONNECTER_EXTENSION2") && !Extension2)
    {
      Extension2 = NoPort;
      EnvoieMessage(EXTENSION2, "OK");
    }
...
void ExecuterCommande() /// A RECODER : TOUTES LES COMMANDES DOIVENT PRENDRE EN COMPTE L'ENVOYEUR, ET L'ADMINISTRATEUR !!! A REFAIRE !!!
{
  ...
  if(Commande==String("OBR_CONNECTER") && Mode==CONFIGURATION)
  {
  ...
  {
    if(param1.toInt()>0 && param1.toInt()<4096 && Connecte)
    {
      digitalWrite(13, LOW);
  ...
  if(Commande==String("OBR_DECONNECTER") && Mode==CONFIGURATION)
  {
    if(Connecte)
    {
      Connecte=0;
